...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        */
        ...
        lblLeftHeader = [[UILabel alloc] initWithFrame:CGRectZero];
        lblLeftHeader.textColor = self.cellTextColor;
        lblLeftHeader.textAlignment = UITextAlignmentCenter;
		[lblLeftHeader sizeToFit];
        [vertexView addSubview:lblLeftHeader];
...
- (void)reset {
    ...
    lblLeftHeader.backgroundColor = vertexView.backgroundColor;
    lblLeftHeader.textColor = self.headerTextColor;
    if ([self.datasource respondsToSelector:@selector(titleForHeaderInTableView:)]) {
        lblLeftHeader.text = [self.datasource titleForHeaderInTableView:self];
    } else {
        lblLeftHeader.text = @"";
    }
...
- (UITableViewCell *)leftHeaderTableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	UIView *view = [[UIView alloc] initWithFrame:CGRectInset(rect, 2.f, 2.f)];
	view.clipsToBounds = YES;
...
- (NSUInteger)rowsInSection:(NSUInteger)section {
    ...
    if ([leftHeaderDataArray count] > section) {
        rows = [[leftHeaderDataArray objectAtIndex:section] count];
    }
	return rows;
}
