...
@interface LXActionSheet ()<UIGestureRecognizerDelegate>{
...
- (id)initWithTitle:(NSString *)title delegate:(id<LXActionSheetDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle destructiveButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSString *)otherButtonTitles, ...
{
    self = [super init];
    if (self) {
...
-(void)setCancelButtonColor:(UIColor *)color titleColor:(UIColor *)titleColor  icon:(UIImage *)image{
    [self setButtonIndex:_postionIndexNumber -1 buttonColor:color titleColor:titleColor icon:image];
}

-(void)setdestructiveButtonColor:(UIColor *)color titleColor:(UIColor *)titleColor  icon:(UIImage *)image{
    [self setButtonIndex:0 buttonColor:color titleColor:titleColor icon:image];
}

-(void)setButtonIndex:(NSInteger)index buttonColor:(UIColor *)buttonColor titleColor:(UIColor *)titleColor icon:(UIImage *)image{
    UIButton *btn = (UIButton *)[_buttonArray objectAtIndex:index];
    if (btn) {
        if (buttonColor) {
...
//    [self.backGroundView addGestureRecognizer:tapGesture];
    ...
    if (destructiveButtonTitle) {
        ...
        [_buttonArray addObject:destructiveButton];
        ...
        if (otherButtonTitlesArray.count > 0) {
            ...
            if (self.isHadTitle == YES || self.isHadDestructionButton == YES) {
                for (int i = 0; i<otherButtonTitlesArray.count; i++) {
                    ...
                    [self.backGroundView addSubview:otherButton];
                    [_buttonArray addObject:otherButton];
                    self.postionIndexNumber++;
                }
            }
        }
    ...
    if (cancelButtonTitle) {
        ...
        [_buttonArray addObject:cancelButton];
        self.postionIndexNumber++;
    }
...
- (void)clickOnButtonWith:(UIButton *)button
{
    if (self.isHadDestructionButton == YES) {
        if (self.delegate) {
            if (button.tag == 0) {
                if ([self.delegate respondsToSelector:@selector(lxactionSheetDestructive:)] == YES){
                    [self.delegate lxactionSheetDestructive:self];
                }
            }
        }
    }
    ...
    if (self.isHadCancelButton == YES) {
        if (self.delegate) {
            if (button.tag == self.postionIndexNumber-1) {
                if ([self.delegate respondsToSelector:@selector(lxactionSheetCancel:)] == YES) {
                    [self.delegate lxactionSheetCancel:self];
                }
            }
        }
    }
    ...
    if (self.delegate) {
        if ([self.delegate respondsToSelector:@selector(lxactionSheet:clickedButtonAtIndex:)] == YES) {
            [self.delegate lxactionSheet:self clickedButtonAtIndex:button.tag];
        }
    }
